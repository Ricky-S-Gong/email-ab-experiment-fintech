...
\begin{table}[H]
  ...
  \begin{tabular}{@{}lccccr@{}}
    \toprule
    \textbf{Segment Name} & \textbf{Appr. <6M} & \textbf{Bank Linked} &
    \textbf{Active 20d} & \textbf{Trade 5d} & \textbf{Total Pool} \\
    \midrule
...
The analysis uses five structured datasets:

\begin{table}[H]
  \centering
  \caption{Dataset Summary}
  \label{tab:datasets}
  \begin{tabular}{@{}L{4.5cm}rL{6cm}@{}}
    \toprule
    \textbf{File} & \textbf{Shape} & \textbf{Description} \\
    \midrule
    \texttt{sample\_segment\_groups.csv}   & 12 $\times$ 8    & Segment definitions with user counts \\
    \texttt{sample\_uuid\_email\_order.csv} & 480,000 $\times$ 13 & Per-user randomized email delivery schedule \\
    \texttt{email\_events.csv}             & 9,553,330 $\times$ 6 & Raw email interaction events from delivery system \\
    \texttt{user\_events.csv}             & 480,000 $\times$ 22 & User-level email status and outcome metrics \\
    \texttt{control\_groups\_rate.csv}     & 12 $\times$ 6    & Pre-computed control group baseline rates \\
    \bottomrule
  \end{tabular}
\end{table}
...
\textbf{Email interaction status} (in \texttt{user\_events.csv}): For each of the 10
email templates, users carry one of the following status values: \texttt{delivered},
\texttt{open}, \texttt{unsubscribe}, \texttt{spamreport}, or \texttt{NaN} (not yet
sent). The status represents the \textit{highest-intent action} taken on that email.

\textbf{Outcome variables}:
\begin{itemize}[leftmargin=2em]
  \item \texttt{first\_funded\_at}: Timestamp of first funding event (primary outcome)
  \item \texttt{first\_linked\_bank\_account\_at}: Timestamp of bank account linkage
    (leading indicator for funding)
  \item \texttt{5d/20d\_trading\_avg\_event\_count}: Trading activity proxies
    (user engagement signal)
\end{itemize}

\textbf{Email event log} (in \texttt{email\_events.csv}): Raw events from the
Post-Office delivery system totaling 9.55M rows, including:

\begin{table}[H]
  \centering
  \caption{Email Event Volume Breakdown}
  \label{tab:events}
  \begin{tabular}{@{}lrl@{}}
    \toprule
    \textbf{Event Type} & \textbf{Count} & \textbf{Meaning} \\
    \midrule
    processed    & 4,208,119 & Accepted for delivery \\
    delivered    & 4,191,336 & Successfully delivered to inbox \\
    open         &   870,651 & Recipient opened the email \\
    dropped      &   241,805 & Suppressed (unsubscribe list, etc.) \\
    bounce       &    18,166 & Hard or soft bounce \\
    deferred     &    12,059 & Temporarily delayed delivery \\
    unsubscribe  &    10,894 & User opted out of email \\
    spamreport   &       300 & User reported as spam \\
    \bottomrule
  \end{tabular}
\end{table}
